...
let dbDriverNoteLogged = false;

function logDbDriverOnce(message) {
  if (dbDriverNoteLogged) return;
  dbDriverNoteLogged = true;
  console.warn(message);
}

function prefersSqlJs() {
  if (process.env.FORCE_SQLJS === '1') return true;
  if (process.env.FORCE_NATIVE_SQLITE === '1') return false;
  if (process.env.NODE_ENV === 'production') return true;
  if (process.env.PASSENGER_APP_ENV) return true;
  return false;
}
...
function tryOpenNative(path) {
  if (prefersSqlJs()) return null;
  let Database;
  try {
    Database = require('better-sqlite3');
  } catch {
    logDbDriverOnce('[db] using sql.js (better-sqlite3 not installed)');
    return null;
  }
  try {
    return wrapNative(new Database(path));
  } catch {
    logDbDriverOnce('[db] using sql.js (native SQLite bindings unavailable on this host)');
    return null;
  }
}
...
async function openDatabase(path) {
  ...
  if (!native) {
    const driverLabel = label.includes('vendor') ? 'sql.js (bundled)' : 'sql.js';
    logDbDriverOnce(`[db] using ${driverLabel}`);
  }
