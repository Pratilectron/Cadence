...
function parseStoredAvatar(raw) {
  ...
  try {
    ...
    if (data?.kind === 'preset' && /^mark-(0[1-9]|[12]\d|3[0-6])$/.test(data.id)) {
      return { kind: 'preset', id: data.id };
    }
...
function rowToUser(row) {
  ...
  return {
    ...
    bio: row.bio || '',
    avatar: parseStoredAvatar(row.avatar),
...
function initSchema() {
  ...
  ensureColumn('room_meta', 'code', "TEXT NOT NULL DEFAULT ''");
  ensureColumn('room_messages', 'updated_at', 'INTEGER NOT NULL DEFAULT 0');
...
async function initDb() {
  ...
  if (!initPromise) {
    initPromise = (async () => {
      ...
      if (process.env.NODE_ENV !== 'production') {
        console.log(`[db] ready (${db.driver}) at ${DB_FILE}`);
      }
      initSchema();
...
function upsertUser(user) {
  ...
  getDb().prepare(`
    INSERT INTO users (id, username, algo, password_hash, salt, super_admin, display_name, bio, avatar, preferences, created_at)
    VALUES (@id, @username, @algo, @password_hash, @salt, @super_admin, @display_name, @bio, @avatar, @preferences, @created_at)
...
function loadRoomStoreObject() {
  const rows = getDb().prepare('SELECT room_name, roles_json, member_roles_json, code FROM room_meta').all();
  const store = {};
  for (const row of rows) {
    store[row.room_name] = {
...
function saveRoomStoreObject(store) {
  ...
  const ins = getDb().prepare(`
    INSERT INTO room_meta (room_name, roles_json, member_roles_json, code) VALUES (?, ?, ?, ?)
  `);
  const run = getDb().transaction((data) => {
    ...
    for (const [roomName, meta] of Object.entries(data)) {
      ins.run(
        ...
        JSON.stringify(meta.memberRoles || {}),
        meta.code || '',
      );
    }
  });
...
function insertRoomMessage(roomName, payload) {
  ...
  getDb().prepare(`
    INSERT OR REPLACE INTO room_messages (id, room_name, payload_json, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?)
  `).run(payload.id, roomName, JSON.stringify(payload), createdAt, updatedAt);
}

function listRoomMessages(roomName, { since = 0, limit = 200 } = {}) {
  const rows = getDb().prepare(`
    SELECT payload_json FROM room_messages
    WHERE room_name = ? AND (created_at > ? OR updated_at > ?)
    ORDER BY created_at ASC
    LIMIT ?
  `).all(roomName, since, since, limit);
