...
const { projectMessage, visibleMessages } = require('./message-visibility');

function jsonError(res, jsonResponse, status, message) {
  jsonResponse(res, status, { error: message });
}

function roomRolesFor(ctx, room, user) {
  if (!room || typeof ctx.serializeRolesForClient !== 'function') return null;
  const payload = ctx.serializeRolesForClient(room, user.id || null);
  if (user.id && ctx.isSuperAdmin?.(user)) {
    payload.myPermissions = {
      ...payload.myPermissions,
      ADMINISTRATOR: true,
      SEND_MESSAGES: true,
      ATTACH_FILES: true,
...
function buildHistory(ctx, roomName, user) {
  ...
  const cfg = ctx.cfg();
  const canManage = Boolean(user.id) && ctx.userHasPermission(room, user.id, 'MANAGE_MESSAGES');
  const allMessages = visibleMessages(
    ctx.listRoomMessages(roomName, { since: 0, limit: cfg.maxMessagesPerRoom }),
    canManage,
  );
...
async function handleChatHello(req, res, jsonResponse, ctx) {
  ...
  jsonResponse(res, 200, {
    ...
    roomRoles: roomRolesFor(ctx, room, user),
    roomType: room.type,
...
async function handleChatPoll(req, res, jsonResponse, ctx) {
  ...
  const canManage = Boolean(user.id) && ctx.userHasPermission(room, user.id, 'MANAGE_MESSAGES');
  const project = (rows) => rows.map((message) => projectMessage(message, canManage)).filter(Boolean);
  let messages = project(ctx.listRoomMessages(roomName, { since, limit: 100 }));
  const includeMeta = params.get('meta') === '1' || since === 0;
  if (params.get('wait') === '1' && messages.length === 0) {
    const deadline = Date.now() + 20000;
    while (Date.now() < deadline && messages.length === 0) {
      if (req.destroyed || res.writableEnded) break;
      await new Promise((resolve) => setTimeout(resolve, 350));
      messages = project(ctx.listRoomMessages(roomName, { since, limit: 100 }));
    }
  }
  const cursor = messages.reduce((max, message) => Math.max(max, message.updatedAt || message.ts || 0), since);
...
async function handleChatMessageAction(req, res, jsonResponse, ctx) {
  if (req.method !== 'POST') {
    jsonError(res, jsonResponse, 405, 'Method not allowed.');
    return true;
  }
  ...
  const roomName = ctx.sanitizeRoomName(body.room) || ctx.cfg().defaultRooms[0] || 'General';
  if (!ctx.rooms[roomName]) {
    jsonError(res, jsonResponse, 404, 'Room not found.');
    return true;
  }

  const result = ctx.applyMessageAction({
    roomName,
  ...
  if (result.error) {
    jsonError(res, jsonResponse, result.status || 400, result.error);
    return true;
  }
  if (result.event) {
    ctx.logHttpActivity(result.event, user, { room: roomName, messageId: body.messageId });
  }
  jsonResponse(res, 200, { ok: true, message: result.message });
  return true;
}
